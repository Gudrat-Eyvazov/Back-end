...
(function (factory) {
	...
	$.BigVideo = function(options) {
	    ...
	    var video_arr = [],
	    i_video_arr = [],
	    video_length = $("#index_video .data span").length,
	    op = 0;
	    for (var i = 0; i < video_length; i++) {
	        video_arr.push($("#index_video .data span").eq(i).data("youtube"));
	        i_video_arr.push($("#index_video .data span").eq(i).data("image"));
	    }
		...
		function nextMedia() {
			...
			op++;
			if (op == video_length) { op = 0}
			$("#index_video a.play").attr("data-video", video_arr[op]);
			$("#index_video").attr("style", "background: url("+i_video_arr[op]+")");
		}
